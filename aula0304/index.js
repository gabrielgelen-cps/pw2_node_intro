...
function buildAccount(){
    inquirer.prompt([
        {
            name: 'accountName',
            message: 'Forneça o nome para sua conta no Bank Accounts App!'
        }
    ]).then((answer) => {
...
function withdraw(){
    inquirer.prompt([
        ...
        if(!checkAccount(accountName)){
            console.error(chalk.bgRed.white("Essa conta não existe!"))
            return withdraw()
        }
...
function getAccountBalance(){
    inquirer.prompt([
        ...
        if (accountData.balance<0){
            console.info(chalk.bgGreen.white(`Limite especial de: ${accountData.balance + accountData.limit}`))
        } else {
